export const RulesConfig = {
  red: {
    ...
    rules: {
      low: [
        ...
        "Jeder Testcode muss mindestens einmal aus den richtigen Gründen fehlschlagen (NotImplementedException zählt z.B. nicht!). D.h. dass man auch in diesem Schritt Produktionscode schreiben darf, um zu garantieren, dass der Code aus den richtigen Gründen fehlschlägt.",
        "Falls möglich verwende das OneToMany Pattern. D.h. wenn das Problem für eine Collection von Items gelöst werden sollte, schreibe zunächst einen Test der das Problem für ein einzelnes Item löst. Erst wenn ein solcher Test durch einen kompletten Zykel durchlaufen hat, darf der Test durch einen Test für eine Collection ersetzt werden."
      ],
      medium: [
    ...
    tips: {
      low: [
        ...
        "Tests sollten keine externen Abhängigkeiten haben bzw. nicht von der Umgebung abhängen in der der Test läuft. => Der Test muss im Arrange Step alle Abhängigkeiten und Daten selbst erzeugen, die der Test benötigt und die Reihenfolge der Testausführungen muss irrelevant sein.",
  ...
  green: {
    ...
    tips: {
      low: [
        "Lasse dich nicht durch Details wie gutes Naming, Klassen verschieben, richtige Methodensignaturen oder andere Refactoring Aufgaben ablenken. Diese dürfen erst im Refactoring Step angewendet werden. In diesem Step geht es nur darum mit möglichst wenig Code alle Tests zu bestehen.",
      ],
      medium: [
        "Lasse dich nicht durch Details wie gutes Naming, Klassen verschieben, richtige Methodensignaturen oder andere Refactoring Aufgaben ablenken. Diese dürfen erst im Refactoring Step angewendet werden. In diesem Step geht es nur darum mit möglichst wenig Code alle Tests zu bestehen.",
      ],
  ...
  refactor: {
    ...
    rules: {
      low: [
        "Dieser Zykel kann erst beendet werden, wenn alle Tests nach dem Refactoring immer noch bestanden sind.",
    ...
    tips: {
      low: [
        ...
        "Testklassen können verschachteln werden, um die Namen der Testmethoden zu verkürzen und die Tests besser zu strukturieren.",
        ...
        "Mache dir jetzt Gedanken um sinnvolle Namen, der Position von Klassen und richtigen Methodensignaturen oder der Einführung von Enums oder neuen Klassen und Methoden. Beachte dass nicht alles beim ersten Refactoring perfekt gemacht werden muss. Namen können z.B. später noch überarbeitet werden."
      ],
      medium: [
        "Beachte die S.O.L.I.D Prinzipien. Besonders sollte darauf geachtet werden, dass jede Klasse und Methode genau einen Grund hat sich zu verändern.",
        "Wenn man OneToMany verwendet, und dafür auch die API einer Klasse verändern muss (z.B. Customer zu Customer[] ändern), kann man erstmal einen weiteren Overload für die Methode mit der geänderten API definieren und die alte API temporär behalten",
